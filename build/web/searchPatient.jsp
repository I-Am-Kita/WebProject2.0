...
function changeInput(){                    //This loads combo box or textbox according to the selection
    ...
    combo.append("<option>circulatorySystem</option>");
    combo.append("<option>digestiveSystem</option>");
    combo.append("<option>endocrineSystem</option>");
    combo.append("<option>integumentarySystem</option>");
    combo.append("<option>immuneSystem</option>");
    combo.append("<option>muscularSystem</option>");
    combo.append("<option>nervousSystem</option>");
    combo.append("<option>urinarySystem</option>");
    combo.append("<option>reproductiveSystem</option>");
    combo.append("<option>respiratorySystem</option>");
    combo.append("<option>skeletalSystem</option>");
...
function loadXMLDoc()
{
	...
	if(x=='1'){
			...
			xmlhttp.onreadystatechange=function()
 			...
 			xmlhttp.open("GET", "member_search.php?searchType=" + searchType + "&searchInput=" + searchInput, true);  //Sends a request to member_search.php with member type as search input
		        xmlhttp.send();
						...
						var searchType = document.getElementById("searchtype").value;
			var searchInput = document.getElementById("nic").value;                        //Get search input for search ny NIC
			var xmlhttp;
			if (window.XMLHttpRequest)
 			 {                                                                             // code for IE7+, Firefox, Chrome, Opera, Safari
 			 xmlhttp=new XMLHttpRequest();
			...
			xmlhttp.onreadystatechange=function()
 			...
 			xmlhttp.open("GET", "member_search2.php?searchType=" + searchType + "&searchInput=" + searchInput, true);  //Sends a request to member_search2.php with member NIC as search input
		        xmlhttp.send();
			...
			var searchInput = document.getElementById("surname").value;
			var xmlhttp;
			if (window.XMLHttpRequest)
 			 {                                                                              // code for IE7+, Firefox, Chrome, Opera, Safari
 			 xmlhttp=new XMLHttpRequest();
			...
			xmlhttp.onreadystatechange=function()
 			...
 			xmlhttp.open("GET", "member_search3.php?searchType=" + searchType + "&searchInput=" + searchInput, true); //Sends a request to member_search3.php with member email as search input
		        xmlhttp.send();
 ...
 <script type="text/javascript">
                                 ...
                                 function confirmation() {
                                  var status=false;
                                var x=window.confirm("You are about to remove this user from the system");
                                if (x){
...
<body>
    <div id="wrapper">
        ...
        <font face="Agency FB" color="black" size="6px" ><Strong>Patient Search Panel</Strong></font>
        <br/><br/>
		...
		<div  style="text-align:center;">
								...
								<select id='searchtype' class="small select" name="dropdown" onchange="changeInput();">    //Creates dropdown menu with 3 options and loads combobox or textbox according to the choise
									...
									<option value="1" >Search by Organ Type</option>
									<option value="2" >Search by User NIC</option>
									<option value="3" >Search by Surname</option>
